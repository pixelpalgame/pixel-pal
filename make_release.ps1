...
if ($input -eq '') {
    ...
    try { $newVersion = [System.Version]$input }
...
# ── Build the zip ────────────────────────────────────────────────
# Everything EXCEPT runtime/, node_modules/, _update*, and build artifacts
    ...
    if ((Get-Item $sourcePath).PSIsContainer) {
        Get-ChildItem $sourcePath -Recurse | Where-Object { -not $_.PSIsContainer } | ForEach-Object {
            ...
            [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
                $zip, $_.FullName, "$entryName/$relative", 'Optimal'
            ) | Out-Null
        }
    } else {
        [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
            $zip, $sourcePath, $entryName, 'Optimal'
        ) | Out-Null
    }
}
...
Write-Host "    1. git add . && git commit -m 'v$newVersion' && git push"
Write-Host "    2. Go to GitHub → Releases → Draft a new release"
Write-Host "    3. Tag: v$newVersion"
Write-Host "    4. Upload pixel-pal.zip as a release asset"
